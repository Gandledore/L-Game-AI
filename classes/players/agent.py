...
class Agent(Player):
    def getMove(self, game: Game) -> action:
        ...
        state = game.state
        bestAction = self.MinimaxSearch(game, state)
        return bestAction

    # function MINIMAX-SEARCH(game, state) returns an action
    def MinimaxSearch(self, game: Game, state) -> action:
    ...
    def MaxValue(self, game, state) -> (int, action):
    ...
    #         v2, a2 <- MIN-VALUE(game, game.RESULT(state, a))
            v2, a2 = self.MinValue(game, game.getSuccessor(state, action))
    #         if v2 > v then
            if v2 > v:
    #             v, move <- v2, a
                v, move = v2, a2
    ...
    def MinValue(self, game, state) -> (int, action):
    #     if game.IS-TERMINAL(state) then return game.UTILITY(state, player), null
        if game.isGoal(state):
    ...
    #         v2, a2 <- MAX-VALUE(game, game.RESULT(state, a))
            v2, a2 = self.MaxValue(game, game.getSuccessor(state, action))
    #         if v2 < v then
            if v2 < v:
    #         v, move <- v2, a
                v, move = v2, a2
